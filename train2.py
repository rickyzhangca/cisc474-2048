...
q = dql()
q.build_network()
outcomes, scores, losses, logs = q.train(episodes=120000, max_replay=6000)
...
save(path='./trained2', name='/losses', lis=losses, mode='.txt')
save(path='./trained2', name='/logs', lis=logs, mode='.txt')
...
for w in weights:
    ...
    save(path='./trained2', name='/' + w, lis=flatten, mode='.csv')
