...
def ricky_conv1_layer1_weights():
    return conv1_layer1_weights
def ricky_conv2_layer1_weights():
    return conv2_layer1_weights
def ricky_conv1_layer2_weights():
    return conv1_layer2_weights
def ricky_conv2_layer2_weights():
    return conv2_layer2_weights
def ricky_fc_layer1_weights():
    return fc_layer1_weights
def ricky_fc_layer1_biases():
    return fc_layer1_biases
def ricky_fc_layer2_weights():
    return fc_layer2_weights
def ricky_fc_layer2_biases():
    return fc_layer2_biases
...
@tf.function
def single_output():
    return model(single_dataset)

# for batch data
@tf.function
def logits():
    return model(tf_batch_dataset)
...
def optimizer(learning_rate,loss):
    return tf.compat.v1.train.RMSPropOptimizer(learning_rate).minimize(loss, global_step=global_step)
...
for ep in range(M):
    ...
    while(finish=='not over'):
        ...
        if(len(replay_memory)>=mem_capacity):
            ...
            final_parameters['conv1_layer1_weights'] = ricky_conv1_layer1_weights()
            final_parameters['conv1_layer2_weights'] = ricky_conv1_layer2_weights()
            final_parameters['conv2_layer1_weights'] = ricky_conv2_layer1_weights()
            final_parameters['conv2_layer2_weights'] = ricky_conv2_layer2_weights()
            ...
            final_parameters['fc_layer1_weights'] = ricky_fc_layer1_weights()
            final_parameters['fc_layer2_weights'] = ricky_fc_layer2_weights()
            final_parameters['fc_layer1_biases'] = ricky_fc_layer1_biases()
            final_parameters['fc_layer2_biases'] = ricky_fc_layer2_biases()
